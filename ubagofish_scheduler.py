...
def is_in_lunch_break(time_val):
    idx = HOURS.index(time_val)
    return lunch_start_idx <= idx < lunch_end_idx

# Remove appointments overlapping lunch
...
# Manage appointments (clear/edit)
...
if action == "Limpiar todo" and st.sidebar.button("Ejecutar"):
    st.session_state.appointments = []
    autosave()
    st.sidebar.success("Se limpiaron todas las citas.")
...
# Tabs for random/manual scheduling
...
with tab2:
    ...
    if st.button("➕ Agendar cita manual"):
        if is_in_lunch_break(hora_manual):
            st.warning("No se pueden agendar reuniones durante el almuerzo.")
        else:
...
# Export schedule to Excel (two sheets: Proveedores & Empresas)
